
### Hi there! 👋

I'm **Vinay Sen** — a curious learner and developer from **India**. 🚀

  Learner & Developer — focused on .NET Core and C#  while building small projects and sharpening practical skills. 

 "Learning one project at a time." 🎯

## About Me


- Bio: "Learning!" — always exploring new tools, patterns and improving through hands-on projects. 📚
- Location: Rewa, India 🇮🇳
- Mid Slow Learner
<!--
Quick snapshot:

- Public repositories: **11**
- Followers: **11**
- Following: **17** -->

Currently learning & focusing on:

- C# (SOLID Principles) 🧠
- ASP.NET Core  🌐

Open to collaborating on learning-focused projects and internships. 🤝

## Skills & Technologies

html,css,cpp,,js,git,github,C#,C++, ASP.NET Core
<!--
## Top Projects

### Top Projects (selected)

1. [JavaScript-DOM-Examples](https://github.com/vinayMsen/JavaScript-DOM-Examples) 🔗
   - Description: Small DOM manipulation and examples focused on learning JavaScript + HTML.
   - Stars: ⭐ 1
   - Primary language: HTML
   - Forks: 0 • Open issues: 0 • Watchers: 1

2. [CPP-Programs](https://github.com/vinayMsen/CPP-Programs) 🔗
   - Description: Learning OOPs — collection of C++ programs and examples.
   - Stars: ⭐ 1
   - Primary language: C++
   - Forks: 0 • Open issues: 0 • Watchers: 1

3. [Firstweb](https://github.com/vinayMsen/Firstweb) 🔗
   - Description: First web page built using HTML and CSS; GitHub Pages enabled.
   - Stars: ⭐ 1
   - Primary language: CSS
   - Forks: 0 • Open issues: 0 • Watchers: 1
   - Live: https://vinayMsen.github.io/Firstweb

4. [Octanet](https://github.com/vinayMsen/Octanet) 🔗
   - Description: Work from a 2-month internship (front-end / styling focus).
   - Stars: ⭐ 1
   - Primary language: CSS
   - Forks: 0 • Open issues: 0 • Watchers: 1

5. [CBTC](https://github.com/vinayMsen/CBTC) 🔗
   - Description: 4-week internship project — C++ focused (includes file handling, cpp17 references).
   - Stars: ⭐ 1
   - Primary language: C++
   - Forks: 0 • Open issues: 0 • Watchers: 1
   - Demo / resources: https://drive.google.com/open?id=16PT_nSC5cQtMm5leuYeE6LEvpbvDzhR-&usp=drive_copy


Notes:
- These projects highlight a focus on fundamentals — web front-end (HTML/CSS) and C++ practice. 🔧 -->

## GitHub Stats 📊

- Public repositories: **11**
- Followers: **11**
- Following: **17**

Badges & cards:

- Followers badge: ![Followers](https://img.shields.io/github/followers/vinayMsen?style=social)
- Repos badge: ![Repos](https://img.shields.io/badge/Public%20Repos-11-blue)

Profile cards:

- GitHub Readme Stats: ![GitHub Stats](https://github-readme-stats.vercel.app/api?username=vinayMsen&show_icons=true&hide_title=true)
- Top Languages: [![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=vinayMsen&layout=compact&theme=tokyonight)](https://github.com/anuraghazra/github-readme-stats)

<!--

## Recent Activity 🔔

Here are the latest public events from the account:

1. CreateEvent — Created branch `main` in repo **[vinayMsen/vinayMsen](https://github.com/vinayMsen/vinayMsen)**
   - When: 2026-03-10T13:16:19Z

2. WatchEvent — Started watching **imvish21/Cryptopod-app**
   - When: 2026-03-10T12:58:24Z
-->


<!--
(These are the most recent recorded events; connect your activity feeds for more updates and live badges.)

## Latest Blog Posts

### Latest Blog Posts / Articles ✍️

- No public blog posts were found on GitHub. 
- LinkedIn (profile / posts): https://www.linkedin.com/in/vinaysen/ — use this to view posts, articles or updates from Vinay.

If you maintain a personal blog or publish articles, you can add RSS or link them here for automatic listing.

## Connect with Me

### Let's connect! 🤝

- GitHub: [@vinayMsen](https://github.com/vinayMsen) 🐙
- LinkedIn: [Vinay Sen](https://www.linkedin.com/in/vinaysen/) 🔗
-->

Badges:

- LinkedIn: ![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-blue?logo=linkedin)
- GitHub: ![GitHub](https://img.shields.io/badge/GitHub-Profile-black?logo=github)
